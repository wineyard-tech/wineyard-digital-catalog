...
import type { EnquiryStatus, EnquiryListItem } from '@/types/catalog'
...
export async function GET(request: NextRequest) {
  ...
  const items: EnquiryListItem[] = page.map((row) => {
    ...
    return {
      ...
      total: row.total,
      item_count: lineItems.reduce((s, i) => s + (i.quantity ?? 0), 0),
      status: computeStatus({
        status: row.status,
        created_at: row.created_at,
        converted_to_salesorder_id: row.converted_to_salesorder_id,
      }),
    }
  })

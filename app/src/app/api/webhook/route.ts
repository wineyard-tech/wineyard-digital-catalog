...
export async function POST(request: NextRequest) {
  ...
  // Meta requires 200 for all valid signed webhooks — errors are caught and logged
  try {
    ...
    if (recent) {
      // Silently acknowledge — link already sent recently
      return new NextResponse('OK', { status: 200 })
    }
    ...
    if (!contact) {
      // Not in Supabase — check Zoho Books
      const zohoContact = await getContactByPhone(phone)

      if (zohoContact) {
        // Lazy insert into contacts
  ...
  // Always 200 for valid Meta webhooks
  return new NextResponse('OK', { status: 200 })
}
